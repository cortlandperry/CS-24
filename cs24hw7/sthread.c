...
void sthread_start(int timer) {
    ...
    /* whenever __sthread_start is called, we will enter the scheduler, so we
    must lock our thread so we do not corrupt any information before entering.
    We also want to make sure we start the timer when we start the thread */
    ...
    /* we have finished entering the scheduler and calling start, so we unlock
    our code to make it atomic */
    __sthread_unlock();
}
...
void __sthread_finish(void) {
    ...
    /* whenever sthread switch is called, we will enter the scheduler, so we
    must lock our thread so we do not corrupt any information before entering.
    However, we do not want our current->state to change by a timer
    interrupt before we set it to finished, so we must call this lock
    before the contents of this function */
    __sthread_lock();
    ...
    /* once we have finished the context switch, we can unlock our lock because
    we have atomically run our scheduler */
    __sthread_unlock();
...
void sthread_yield() {
    ...
    /* we have finished our context switch, and thus we can now unlock our lock
...
void sthread_block() {
    ...
    /* whenever sthread start is called, we will enter the scheduler, so we
    must lock our thread so we do not corrupt any information before entering
    we need to make sure our current state is not changed after we set it to
    blocked, so __sthread_lock is called before that call */
    __sthread_lock();

    current->state = ThreadBlocked;
    __sthread_switch();

    /* we will unlock our state because we have finished the switch
    that needs to be run atomically in sthread_switch */
    __sthread_unlock();
}
...
void sthread_unblock(Thread *threadp) {
    ...
    /* don't want to remove our thread in our memory queue from blocked and
    then also not add it to our ready queue, so we must lock our context
    here. We do not want to be interruptted before the thread is re-queued */
    __sthread_lock();
    ...
    /* unlock this once the thread is enqueued in the ready_queue, becuase
    we will not have any corrupted memory any more */
    __sthread_unlock();
}
