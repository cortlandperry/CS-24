...
typedef struct multimap_node {
    ...
    /* indicates the pointer to the array that holds all the nodes */
    struct multimap_node *node_array;
...
void mm_add_value(multimap *mm, int key, int value) {
    ...
    node->values = realloc(node->values, sizeof(multimap_value)*(node->size + 1));
