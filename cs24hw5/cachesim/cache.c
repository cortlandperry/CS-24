...
unsigned char cache_read_byte(membase_t *mb, addr_t address) {
    ...
    /* update recent for the cacheline that we are reading from */
...
void cache_write_byte(membase_t *mb, addr_t address, unsigned char value) {
    ...
    /* update recent for the cacheline that we are wrote to */
    p_line->recent = clock_tick();
 ...
 */
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
    /* this mask gives us the last digits of our address, all the ones that
    contain the block_offset */
    addr_t mask = ((p_cache->block_size) - 1);
    return (mask & address);
}
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    /* set offset using the function we just made */
    *offset = get_offset_in_block(p_cache, address);
    /* shift our address so it no longer has the block offset info, and the
    set address info will be at the end of the block */
    address = address >> (p_cache->block_offset_bits);
    /* get the mask to get the last digits */
    uint32_t setmask = (p_cache->num_sets - 1);
    /* use our mask to get the value of set */
    *set = address & setmask;

    /* shift our address again to then have the address only be representing the
    tag */
    address = address >> (p_cache->sets_addr_bits);
    *tag = address;
 ...
 */
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    /* this mask is: 11.....100..00 where 0's are all the block offset bits
    this gets us all the information except for the block offset info */
    uint32_t mask = ~(p_cache->block_size - 1);
    return (mask & address);
}
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
                                      addr_t tag, addr_t set_no) {
    //shift tag all the way over, with block and set_addr being 0
    tag = tag << (p_cache->sets_addr_bits);
    tag = tag << (p_cache->block_offset_bits);
    //shift set_no to be the middle bits, with block_offset being 0
    set_no = set_no << (p_cache->block_offset_bits);
    return (set_no + tag);
}
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
    ...
    cacheline_t *current = p_set->cache_lines;
    int i;
    ...
    /* loop though the lines in our cache set, and we find the valid cache line
    that has a matching tag to the tag that we are looking for */
    for (i = 0; i < p_set->num_lines; i++) {
        current = p_set->cache_lines + i;
        /* if we find the matching line, set the found_line to it and break */
...
#else
    /* initialize our values, for implementing LRU */
    cacheline_t *current = NULL;
    victim = p_set->cache_lines;

    /* we loop though the different lines in the cache set */
    for (i_victim = 0; i_victim < p_set->num_lines; i_victim++) {
        /* check the current cache */
        current = p_set->cache_lines + i_victim;
        /* if the current cache is not valid, it means its empty so we choose
        that one */
        if (current->valid == 0) {
            victim = current;
            break;
        }
        /* otherwise, we choose the current line if its recent value is less
        than the vicitms recent line */
        else if (current->recent < victim->recent) {
            victim = current;
        }
    }
    /* return the victim we choose */
    return victim;
